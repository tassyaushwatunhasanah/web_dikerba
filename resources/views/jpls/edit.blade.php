...
@section('title', 'Edit Detail JPL')

@section('content_header')
    <h1 class="m-0 text-dark">Edit Data Detail JPL</h1>
@stop

@section('content')
    <form action="/jpls/update/{{$jpl['id']}}" method="post" enctype="multipart/form-data">
        @method('patch')
        @csrf
        <div class="row">
            <div class="col-12">
                ...
                <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('no_pegawai') is-invalid @enderror" id="no_pegawai" placeholder="Nomor Pegawai" name="no_pegawai" value="{{$jpl->pegawai_id}}">
                            @error('no_pegawai') <span class="text-danger">{{$message}}</span> @enderror
                        </div>
                        <div class="mb-3">
                            <input type="hidden" name="jpl_id" value="{{$jpl->jpl_id}}">
                        </div>
                        <div class="form-group">
                            <input type="hidden" id="pegawai_id" name="pegawai_id" value="{{$jpl->pegawai_id}}">
                        </div>
                        <!-- Pegawai -->
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <div class="form-group">
                                ...
                                <select type="text" class="form-control @error('kategori') is-invalid @enderror" id="kategori"  name="kategori">
                                    ...
                                    <option value='Pelatihan' {{old('kategori', $jpl->kategori) == 'Pelatihan' ? "selected" : ""}}>Pelatihan</option>
                                    <option value='IHT' {{old('kategori', $jpl->kategori) == 'IHT' ? "selected" : ""}}>IHT</option>
                                    <option value='Seminar' {{old('kategori', $jpl->kategori) == 'Seminar' ? "selected" : ""}}>Seminar</option>
                                    <option value='Bimtek' {{old('kategori', $jpl->kategori) == 'Bimtek' ? "selected" : ""}}>Bimtek</option>
                                </select>
                                @error('kategori') <span class="text-danger">{{$message}}</span> @enderror
                            </div>
                            <label for="nama_kegiatan">Nama Kegiatan</label>
                            <input type="text" class="form-control @error('nama_kegiatan') is-invalid @enderror" id="nama_kegiatan" placeholder="Nama Kegiatan" name="nama_kegiatan" value="{{$jpl->nama_kegiatan}}">
                            @error('nama_kegiatan') <span class="text-danger">{{$message}}</span> @enderror
                        </div>
                        <div class="form-group">
                            <label for="tempat">Tempat</label>
                            <input type="text" class="form-control @error('tempat') is-invalid @enderror" id="tempat" placeholder="Tempat" name="tempat" value="{{$jpl->tempat}}">
                            @error('tempat') <span class="text-danger">{{$message}}</span> @enderror
                        </div>
                        <div class="form-group">
                            <label for="tgl_mulai">Tanggal Mulai</label>
                            <input type="date" class="form-control @error('tgl_mulai') is-invalid @enderror" id="tgl_mulai" name="tgl_mulai" value="{{$jpl->tgl_mulai->format('Y-m-d')}}">
                            @error('tgl_mulai') <span class="text-danger">{{$message}}</span> @enderror
                        </div>
                        <div class="form-group">
                            <label for="tgl_selesai">Tanggal Selesai</label>
                            <input type="date" class="form-control @error('tgl_selesai') is-invalid @enderror" id="tgl_selesai" name="tgl_selesai" value="{{$jpl->tgl_selesai->format('Y-m-d')}}">
                            @error('tgl_selesai') <span class="text-danger">{{$message}}</span> @enderror
                        </div>
                        <div class="form-group">
                            <label for="jpl">JPL</label>
                            <input type="number" class="form-control @error('jpl') is-invalid @enderror" id="jpl" placeholder="Misal 3" name="jpl" value="{{$jpl->jpl}}">
                            @error('jpl') <span class="text-danger">{{$message}}</span> @enderror
                        </div>
                        <div class="form-group">
                            <label for="no_sertif">No Sertifikat</label>
                            <input type="text" class="form-control @error('no_sertif') is-invalid @enderror" id="no_sertif" placeholder="Misal 0000/0000" name="no_sertif" value="{{$jpl->no_sertif}}">
                            @error('no_sertif') <span class="text-danger">{{$message}}</span> @enderror
                        </div>
                        <div class="form-group">
                            <label for="penerbit">Penerbit Sertifikat</label>
                            <input type="text" class="form-control @error('penerbit') is-invalid @enderror" id="penerbit" placeholder="Penerbit" name="penerbit" value="{{$jpl->penerbit}}">
