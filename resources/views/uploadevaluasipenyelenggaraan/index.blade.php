...
@section('title', 'Kuesioner Evaluasi Penyelenggaraan Pendidikan Di RS. Ernaldi Bahar | Website Dikerba')

@section('content_header')
    <h1 class="m-0 text-dark">List Upload Evaluasi Penyelenggaraan Pendidikan Di RS. Ernaldi Bahar</h1>
@stop
...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
            <div class="card-body">
            <form action="{{route('uploadevaluasipenyelenggaraan.store')}}" method="post" enctype="multipart/form-data">
        @csrf
                        ...
                        <tr>
                            ...
                            <td>
                                @if( in_array(pathinfo($item->file, PATHINFO_EXTENSION), ['png', 'jpg', 'JPEG']))
                                    <img src="{{asset('file_uploadevaluasipenyelenggaraan')}}/{{$item->file}}" style="height: 40px">
                                @else
                                    <img src="https://www.freeiconspng.com/uploads/file-txt-icon--icon-search-engine--iconfinder-14.png"
                                    style="height: 40px">
                                @endif
                            </td>
                            <td align="center"><a href="{{asset('file_uploadevaluasipenyelenggaraan')}}/{{$item->file}}" download>Download</a></td>
